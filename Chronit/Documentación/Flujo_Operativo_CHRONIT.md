...
2. **Configurar la carrera** desde el panel de **Eventos** → `POST http://localhost:5000/api/events/{id}/setup`.
   - Requiere evento `preparada` **y que NO haya carrera activa**.
   - El core limpia el tablero, crea la sesión, inscribe a los pilotos del evento y les asigna el transponder guardado en la configuración del evento.
   - **No inicia** la carrera.
3. **Iniciar** la carrera desde el panel de **Control de Carrera** → `POST http://localhost:5000/api/race/start`.
   - Si el evento **no** está `preparada`, devuelve error 400.
   - El botón «Iniciar carrera» vive SOLO aquí, nunca en el panel de eventos.
4. La carrera corre en vivo (vueltas del lector ESL-400).
5. **Finalizar** la carrera → `POST http://localhost:5000/api/race/finish`.
   - El core calcula la clasificación y **sincroniza `resultados_carrera`** a PostgreSQL (best-effort).
   - Evento → `finalizado`; tickets → `FINALIZADO`.
6. El **display** (`http://localhost:3000/display`) muestra automáticamente los **resultados** de la última carrera (`GET /api/eventos/resultados/ultima-carrera`).
...
| POST | `{{core}}/api/events/{id}/setup` | `{}` | `200 {session_id…}` — requiere evento `preparada` y sin carrera activa. Configura (NO inicia) |
| POST | `{{core}}/api/race/start` | `{}` | `200 {success}` — inicia la carrera desde el Control de Carrera |
...
10. `POST {{core}}/api/events/{id}/setup` (configurar la carrera; NO inicia).
11. `POST {{core}}/api/race/start` (iniciar desde el Control de Carrera).
12. `POST {{core}}/api/race/finish`.
13. `GET /api/eventos/resultados/ultima-carrera` → ver resultados.
14. Verificar en PostgreSQL: `SELECT * FROM resultados_carrera ORDER BY id DESC;`
