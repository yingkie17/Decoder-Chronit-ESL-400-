...
# ============================================
# CONFIGURACIÓN DE BASE DE DATOS DE USUARIOS (LINUX/DOCKER)
# ============================================
...
def init_users_db():
    with get_users_db() as conn:
        ...
        cursor = conn.execute('SELECT id FROM users WHERE username = ?', ('yingkie',))
        if not cursor.fetchone():
            dev_password = '72254631'
            password_hash = generate_password_hash(dev_password)
            conn.execute('''
                INSERT INTO users (username, password_hash, role)
                VALUES (?, ?, ?)
            ''', ('yingkie', password_hash, 'developer'))
            print(f"✅ Usuario yingkie creado (yingkie / {dev_password})")    

        cursor = conn.execute('SELECT id FROM users WHERE username = ?', ('contador',))
        if not cursor.fetchone():
            contador_password = 'contador123'
            password_hash = generate_password_hash(contador_password)
            conn.execute('''
                INSERT INTO users (username, password_hash, role)
                VALUES (?, ?, ?)
            ''', ('contador', password_hash, 'contador'))
            print(f"✅ Usuario contador creado (contador / {contador_password})")    
