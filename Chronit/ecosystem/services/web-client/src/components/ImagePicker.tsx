// =============================================================================
// CHRONIT WEB CLIENT — Selector de imagen (Client Component)
// -----------------------------------------------------------------------------
// Selector reutilizable con:
...
interface Props {
  ...
  /** Relación de aspecto del recorte. 1 = cuadrado (foto), 16/9 = portada. */
  aspect?: number;
...
export default function ImagePicker({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
      <div
        style={{
          width: aspect === 1 ? previewSize : previewSize,
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button type="button" className="btn btn-ghost" disabled={disabled} onClick={() => fileRef.current?.click()}>
            🖼 {label}
          </button>
        </div>
        <span className="muted small">JPG, PNG o WebP · máx. {MAX_IMAGE_MB} MB</span>
        {error && <span style={{ color: 'var(--red)', fontSize: '0.82rem' }}>⚠️ {error}</span>}
      </div>
        ...
        <div className="modal-overlay" onClick={stopCamera}>
          <div className="modal fade-in" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 520, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 16 }}>
              ...
              <button type="button" className="btn btn-accent" onClick={capture}>
                📸 Capturar
              </button>
            </div>
          </div>
      ...
      {cropSrc && (
        <div className="modal-overlay" onClick={closeCrop}>
          <div className="modal fade-in" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 460, textAlign: 'center' }}>
            ...
            <p className="muted small" style={{ marginBottom: 12 }}>Arrastra para mover y usa el zoom para ajustar.</p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button type="button" className="btn" onClick={closeCrop}>
                Cancelar
              </button>
              <button type="button" className="btn btn-accent" onClick={confirmCrop} disabled={!ready}>
                ✓ Confirmar
              </button>
            </div>
          </div>
