...
*/
function raceModeLabel(mode) {
    return normalizeRaceMode(mode) === 'time_attack' ? 'Clasificación' : 'CARRERA';
}
...
async function loadTableroPublicoFromData(session, leaderboard, speedsMap = {}) {
    ...
    listaPilotos.innerHTML = leaderboard.map((driver, idx) => {
        ...
        // TIEMPO REAL ACUMULADO para TIME ATTACK
        let tiempoPrincipal = totalTime;      // Columna TIEMPO
        let tiempoSecundario = individualTime; // Columna T.INDIV

        if (raceMode === 'time_attack') {
            // TIME ATTACK: TIEMPO = running lap time, T.INDIV = tiempo real acumulado
            let lapTimeActual = '--';
            if (driver.last_detection) {
                const now = Date.now();
                const lastDetMs = new Date(driver.last_detection).getTime();
                if (!isNaN(lastDetMs)) {
                    const runningSec = (now - lastDetMs) / 1000;
                    lapTimeActual = runningSec > 0 ? formatRaceClock(runningSec) : '--';
                }
            }
            const tiempoAcumuladoReal = driver.real_total_time != null ? formatRaceClock(driver.real_total_time) : '--';
            tiempoPrincipal = tiempoAcumuladoReal;
...
setInterval(() => {
    tvRotationIndex += 1;
    renderTvLapRotator();  // ✅ Sigue siendo válido
    const clock = document.getElementById('tvClock');
    if (clock) clock.innerText = new Date().toLocaleTimeString();
}, 5000);
...
function renderLiveLeaderboardPublicStyle(leaderboard, session, speedsMap = {}, timesMap = {}) {
    ...
    container.innerHTML = leaderboard.map((driver, idx) => {
        ...
        if (raceMode === 'time_attack') {
            ...
            tiempoPrincipal = individualTime;  // ← CORREGIDO
            tiempoSecundario = totalTime;       // ← CORREGIDO
        }
