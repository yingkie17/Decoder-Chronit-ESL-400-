...
setupNavigation();
    ...
    async function loadAntennaConfig() {
        ...
        if (res) {
            document.getElementById('minSignalSlider').value = res.min_signal || 60;
            document.getElementById('minSignalValue').value = res.min_signal || 60;
    ...
    try {
        ...
        // 1. SNAPSHOT: Guardamos copia de seguridad si hay datos válidos
        const dataHasLeaderboard = data && data.session && Array.isArray(data.leaderboard) && data.leaderboard.length > 0;
        if (dataHasLeaderboard) {
            lastRaceDataSnapshot = buildRaceSnapshot(data);
        }

        // 2. CONGELADO AUTOMÁTICO: Si la sesión ya viene marcada como completada
        if (data && data.session && data.session.status === 'completed' && dataHasLeaderboard) {
            frozenRaceData = buildRaceSnapshot({ ...data, active: true }, 'completed');
            lastRaceDataSnapshot = frozenRaceData;
            raceTimerState.status = 'completed';
            renderRaceClock();
        }

        // 3. AUTO-FINISH POR TIEMPO + PODIO AUTOMÁTICO AL COMPLETAR
        if (data && data.active && data.session) {
            ...
            if (session.status === 'completed' && !winnerModalShown) {
        ...
        // 4. RECUPERAR DE SNAPSHOT (Si la API ya no devuelve nada pero terminó por vueltas)
        if (!frozenRaceData && lastRaceDataSnapshot && isCompletedByLaps(lastRaceDataSnapshot)) {
            frozenRaceData = buildRaceSnapshot(lastRaceDataSnapshot, 'completed');
            raceTimerState.status = 'completed';
            renderRaceClock();
        }

        // 5. DETERMINAR DATA EFECTIVA (API o Datos Congelados)
        if ((!effectiveData || !effectiveData.active || !effectiveData.session) && frozenRaceData) {
            effectiveData = frozenRaceData;
            raceTimerState.status = 'completed';
            renderRaceClock();
        }
        
        // 6. RENDERIZADO DE LA INTERFAZ
        if (effectiveData && effectiveData.active) {
            const session = effectiveData.session;
            const status = session.status || 'pending';
            ...
            if (totalDriversEl) totalDriversEl.innerText = effectiveData.leaderboard?.length || 0;
            
            const finishedDriversEl = document.getElementById('finishedDrivers');
            if (finishedDriversEl) finishedDriversEl.innerText = effectiveData.leaderboard?.filter(d => d.is_finished).length || 0;
            ...
            // Obtener velocidades
            const speedsMap = {};
            if (effectiveData.session?.id && effectiveData.leaderboard) {
                for (const driver of effectiveData.leaderboard) {
                    const lapsWithSpeed = await apiCall(`/api/laps/speed/${effectiveData.session.id}/${driver.driver_id}`);
            ...
            if (tbody) {
                if (effectiveData.leaderboard && effectiveData.leaderboard.length > 0) {
                    tbody.innerHTML = effectiveData.leaderboard.map(d => {
            ...
            // ========== PANEL LIVE (solo si los elementos existen) ==========
            if (document.getElementById('liveLeaderboardPublicStyle')) {
                renderLiveLeaderboardPublicStyle(effectiveData.leaderboard, effectiveData.session, speedsMap);
            }

            if (document.getElementById('liveLeaderName')) {
                const leader = effectiveData.leaderboard?.[0];
            ...
            if (document.getElementById('liveLapsDetail')) {
                await loadLiveLapsDetail(effectiveData.session?.id, effectiveData.leaderboard);
            }

            if (document.getElementById('liveTvLapsRotator')) {
                await loadLiveTvRotator(effectiveData.session?.id, effectiveData.leaderboard);
            }
            
            await loadTableroPublico(effectiveData);
            await loadPublicView(effectiveData);
            await loadTvView(effectiveData);
            ...
            if (leaderboardBodyEl) {
                leaderboardBodyEl.innerHTML = '<table><td colspan="8" style="text-align:center;">Sin carrera activa</td></tr>';
            }
    ...
    // Monitor de USB y Señales (Fuera del try de la sesión para que siempre funcione)
    try {
        ...
        console.log("Error en monitor:", e); 
    }
}
...
//End of LOAD_LIVE_DATA()
        ...
        let speedsMap = {};
            if (data && data.active && data.session && data.leaderboard) {
                for (const driver of data.leaderboard) {
                    ...
                    } catch(e) { console.warn("Error obteniendo velocidad en PublicView:", e); }
                }
        }
    ...
    function showCurrent() {
                ...
                if (drivers?.length) {
                    tbody.innerHTML = drivers.map(d => `
                        <tr>
                            ...
                            <td>${d.transponder_id || '--'}</td>
                            <td>
                               <button class="btn btn-sm" onclick="editDriverMinimal(${d.id}, '${d.name}', '${d.lastname || ''}', ${d.transponder_id})">✏️</button>
        ...
        document.getElementById('addDriverToRaceBtn').onclick = async () => {
            ...
            if (!driverId) { return; }
            const session = await apiCall('/api/session/current');
            if (!session?.active || session?.session?.status === 'completed') { return; }
            const drivers = await apiCall('/api/drivers');
        ...
        async function loadTransponders() {
            ...
            if (detectedTbody) {
                if (detected?.length) {
                    detectedTbody.innerHTML = detected.map(t => `<tr><td>${t.id}</td><td>${t.kart_id || '--'}</td><td>${t.description || '--'}</td><td><button class="btn btn-sm" onclick="assignTransponder(${t.id})">👤 Asignar</button></td></tr>`).join('');
                } else { 
            ...
            if (newId && newId != id) {
                ...
                showModal('Editar Transponder', `¿Cambiar el ID ${id} por ${numId}?`, async () => {
                    ...
                    if (res?.success) {
                        ...
                        showToast('Error', res.error || 'No se pudo actualizar el transponder.', 'error');
                    }
                });
            }
        };
    ...
    window.editTransponder = async (id) => {
        ...
        if (!transponder) {
            showToast('Error', 'Transponder no encontrado.', 'error');
            return;
        }
    ...
    function setupEditModalEvents() {
        ...
        document.getElementById('editModalConfirmBtn').onclick = async () => {
            ...
            if (newId !== oldId) {
                ...
                if (!idRes?.success) {
                    hideLoader();
                    showToast('Error', idRes?.error || 'No se pudo cambiar el ID.', 'error');
                    return;
                }
            }
    ...
    // ========== MODAL CON 3 GANADORES ==========
        ...
        function winnerDetailsText(driver) {
            ...
            const bestSpeed = driver.best_speed ? `${Math.round(driver.best_speed)} km/h` : '--';
            if (mode === 'time_attack') {
              return `Modo Posición | Kart ${kart} | ${laps} v | Acumulado ${total} | Vel.max ${bestSpeed}`;
            }
            return `Modo Time Attack | Kart ${kart} | ${laps} v | Total ${total} | Mejor ${bestLap} | Vel.max ${bestSpeed}`;
        }
    ...
    showModal('🗑️ Eliminar Respaldos Antiguos', 
        ...
        function updateAuthUI() {
            ...
            if (isAuthenticated && currentUser) {
                ...
                // Si está en un panel de admin, cambiar a tablero público
        ...
        function openLoginModal() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    sessionToken = data.session_token;  // ← GUARDAR EN VARIABLE GLOBAL
...
// ========== BOTÓN RESET DE VISTAS (siempre visible) ==========
    document.getElementById('resetVistasBtn').onclick = async () => {
        saveLoginBeforeReload();
        showLoader('Reseteando vistas...');
        
        // Limpiar variables locales que puedan tener datos viejos
        frozenRaceData = null;
        ...
        // Llamar al backend para resetear el estado de la carrera
        const res = await apiCall('/api/race/reset', { method: 'POST' });
        
        hideLoader();
        
        if (res?.success) {
        showToast('✅', 'Vistas reseteadas', 'success');
            await loadLiveData();  // ← En lugar de location.reload()
        } else {
            showToast('❌', res?.error || 'No se pudo resetear', 'error');
        }
    };
 ...
 // ========== SPLASH DE INICIO DE CARRERA ==========
    ...
    function renderLiveLeaderboardPublicStyle(leaderboard, session, speedsMap = {}) {
        ...
        container.innerHTML = leaderboard.map((driver, idx) => {
            ...
            const best = driver.best_lap != null ? formatRaceClock(driver.best_lap) : '--';
            const totalTime = driver.total_time != null ? formatRaceClock(driver.total_time) : '--';
            const individualTime = driver.individual_time_formatted || '--';
