...
def generar_vuelta_simulada():
    ...
    if transponder_id not in PRIMERA_VEZ:
        ...
        add_log(f"🎮 [SIMULACIÓN] Primera detección para {nombre_piloto}")  # ← AGREGAR
        return

    VUELTAS_CARRERA[transponder_id] = VUELTAS_CARRERA.get(transponder_id, 0) + 1
    vuelta_carrera = VUELTAS_CARRERA[transponder_id]
    tiempo_total = tiempo_actual - PRIMER_TIEMPO_SERVIDOR.get(transponder_id, tiempo_actual)

    print(f"\n🎮 [SIMULACIÓN] Vuelta generada para {nombre_piloto}")
    add_log(f"🎮 [SIMULACIÓN] Vuelta generada para {nombre_piloto}")  # ← AGREGAR
    
    print(f"   🏎️  Transponder: {transponder_id}")
    add_log(f"   🏎️  Transponder: {transponder_id}")  # ← AGREGAR
    
    print(f"   ⏱️  Tiempo vuelta {vuelta_carrera}: {lap_time:.3f}s")
    add_log(f"   ⏱️  Tiempo vuelta {vuelta_carrera}: {lap_time:.3f}s")  # ← AGREGAR
    
    print(f"   📊 Tiempo total: {tiempo_total:.3f}s")
    add_log(f"   📊 Tiempo total: {tiempo_total:.3f}s")  # ← AGREGAR
    ...
    if avg_speed:
        ...
        add_log(f"   🏁 Velocidad: {avg_speed:.0f} km/h")  # ← AGREGAR
    ...
    if vuelta_carrera >= LAPS_LIMIT and transponder_id not in DRIVERS_FINISHED:
        ...
        print(f"   🏆 ¡{nombre_piloto} COMPLETÓ LA CARRERA!")
        add_log(f"   🏆 ¡{nombre_piloto} COMPLETÓ LA CARRERA!")  # ← AGREGAR
        ...
        if FIRST_FINISHER is None:
            FIRST_FINISHER = {
                ...
                "total_time": tiempo_total - VUELTA_SALIDA.get(transponder_id, 0),
            }
    ...
    if os.path.exists(RACE_COMMAND_FILE):
        ...
        try:
            ...
            if comando.get("action") == "new_race":
                ...
                print(
                    f"🎯 Modo de carrera: {comando.get('race_mode', 'position')}"
                ...
                # Limpiar estado actual (NO hard_reset_all_data)
                RACE_ACTIVE = False
                RACE_PAUSED = False
                DRIVERS_FINISHED = set()
                VUELTA_BASE = {}
                ...
                print(f"📊 Pilotos restaurados: {len(comando['race_drivers'])}")
                return True

            elif comando.get("action") == "clear_all":
                print(f"\n⚠️ REINICIO FORZADO TOTAL SOLICITADO")

                from database import hard_reset_all_data
                ...
                print(f"✅ Sistema reiniciado desde cero")
                return True
...
def procesar_cadena_esl400(raw_data):
    ...
    try:
        ...
        if vuelta_carrera >= LAPS_LIMIT:
            print(f"\n🏆 ¡{nombre_piloto} HA COMPLETADO LA CARRERA!")
            DRIVERS_FINISHED.add(transponder_id)

            if FIRST_FINISHER is None:
                total_time_calc = tiempo_total_segundos - VUELTA_SALIDA.get(
                    transponder_id, 0
                )
                if total_time_calc < 0:
                    print(
                        f"   ⚠️ Tiempo negativo ({total_time_calc:.3f}s) - corrigiendo"
                    )
                    total_time_calc = tiempo_total_segundos

                FIRST_FINISHER = {
                    "driver_id": driver["id"],
                    "transponder_id": transponder_id,
                    "finish_time": tiempo_total_segundos,
                    "total_time": total_time_calc,
                }
                print(
                    f"   ⏱️ Tiempo total del ganador: {FIRST_FINISHER['total_time']:.3f}s"
                )

            # Guardar tiempo de finalización en la BD
            finish_total = tiempo_total_segundos - VUELTA_SALIDA.get(transponder_id, 0)
            if finish_total < 0:
                finish_total = tiempo_total_segundos
            update_driver_finish_time(SESSION_ID, driver["id"], finish_total)

            print(
                f"   ✅ {nombre_piloto} agregado a finalizados ({len(DRIVERS_FINISHED)}/{len(RACE_DRIVERS)})"
            )

            if len(DRIVERS_FINISHED) == len(RACE_DRIVERS):
                print("\n" + "=" * 60)
                print("🏁 ¡CARRERA COMPLETADA POR TODOS LOS PILOTOS!")
                print("=" * 60)
                ...
                if SESSION_ID:
                    winner_id = FIRST_FINISHER["driver_id"] if FIRST_FINISHER else None
                    winner_time = (
                        FIRST_FINISHER["total_time"] if FIRST_FINISHER else None
                    )

                    if winner_time is not None and winner_time < 0:
                        print(
                            f"   ⚠️ winner_time negativo ({winner_time:.3f}s) - corrigiendo"
                        )
                        winner_time = abs(winner_time)

                    update_race_status(SESSION_ID, "completed", winner_id, winner_time)
                    print("📢 Estado actualizado a COMPLETED. Dashboard congelado.")

        print("-" * 50)

        if SESSION_ID and not es_primera_vez:
            is_valid_race_lap = vuelta_carrera > 0 and vuelta_carrera <= LAPS_LIMIT
...
def listen_chronit():
    ...
    if is_simulation_mode():
        print(
            "🎮 [SIMULACIÓN] Modo simulación activado - Generando vueltas automáticas..."
        )
        print(
            "   Las vueltas se generarán cada 5 segundos cuando la carrera esté activa"
        )
        ...
        while True:
            ...
            # Esperar entre generaciones de vueltas (5 segundos)
            time.sleep(5)
        # Nota: En modo simulación, nunca sale de este bucle
        return
