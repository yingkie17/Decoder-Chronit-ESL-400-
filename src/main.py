...
def check_race_commands():
    global RACE_ACTIVE, RACE_PAUSED, DRIVERS_FINISHED, VUELTA_BASE, LAST_LAP_TIME, SESSION_ID, FIRST_FINISHER, VUELTAS_CARRERA, PRIMERA_VEZ, VUELTA_SALIDA, LAPS_LIMIT
    ...
    if os.path.exists(RACE_COMMAND_FILE):
        ...
        try:
            ...
            if comando.get('action') == 'new_race':
                ...
                print(f"✅ Nueva carrera creada: {comando['race_name']} ({comando['laps_limit']} vueltas)")
...
def procesar_cadena_esl400(raw_data):
    ...
    try:
        ...
        if time_source == 'decoder':
            ...
            tiempo_total_segundos = momento_deteccion - PRIMER_TIEMPO_SERVIDOR[transponder_id]
            ...
            if time_source == 'server' and transponder_id in LAST_LAP_TIME:
                ...
                if tiempo_ultima_vuelta > 0:
                    ...
                    if lap_candidate < min_lap_time and lap_candidate > 0:
                        print(f"   🔇 Señal fantasma detectada ({lap_candidate:.2f}s < {min_lap_time}s) - IGNORADA")
                        print("-" * 50)
                        return None
