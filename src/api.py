...
def race_start():
    # Verificar rol del usuario (desde la sesión Flask)
    user_role = session.get('role', 'admin')
    
    # Si NO es desarrollador, verificar que el decoder esté conectado
    if user_role != 'developer':
        # Verificar puerto serial
        port_exists = os.path.exists('/dev/ttyUSB0') or os.path.exists('/dev/ttyACM0')
        
        # También verificar si hay un flag de simulación (por si acaso)
        ...
        if not port_exists and not simulation_active:
            return jsonify({
                'success': False, 
                'error': 'Decoder no conectado. Conecta el hardware o usa usuario "developer" para modo simulación.'
            }), 400
