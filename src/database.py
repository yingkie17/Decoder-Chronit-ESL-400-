...
def init_db():
    ...
    with get_db() as conn:
        ...
        ''')
        try:
            conn.execute('ALTER TABLE circuit_config ADD COLUMN track_type TEXT DEFAULT "karting"')
        except sqlite3.OperationalError:
            pass
        # ========== NUEVAS COLUMNAS ==========
        try:
...
def reset_transponder_health(transponder_id):
    with get_db() as conn:
        ...
        cursor = conn.execute(
            'UPDATE transponders SET times_detected = 0 WHERE id = ?',
            (transponder_id,)
        )
...
def start_new_session(circuit_name, laps_limit, race_mode='position', time_limit_seconds=0):
    with get_db() as conn:
...
def save_lap(session_id, driver_id, transponder_id, physical_laps, lap_number, total_seconds, lap_seconds, signal_h, signal_l, position=None, gap_to_leader=None, is_last_lap=False):
    with get_db() as conn:
        ...
        ''', (s_id, driver_id, transponder_id, lap_number, physical_laps, 
              ...
              signal_h, signal_l, is_last_lap, avg_speed))
...
def update_driver_finish_time(session_id, driver_id, finish_total_seconds):
    ...
    with get_db() as conn:
        ...
        conn.execute('''
            UPDATE race_drivers 
            SET finished = 1, total_time = ?
            WHERE session_id = ? AND driver_id = ?
        ''', (finish_total_seconds, s_id, driver_id))
        return True
...
def get_leaderboard_with_details(session_id):
    with get_db() as conn:
        ...
        # Obtener el modo de carrera de la sesión
        session_info = conn.execute('SELECT race_mode, laps_limit FROM race_sessions WHERE id = ?', (s_id,)).fetchone()
        race_mode = normalize_race_mode(session_info['race_mode']) if session_info else 'position'
        laps_limit = session_info['laps_limit'] if session_info and session_info['laps_limit'] else 10
        ...
        results = conn.execute('''
            SELECT 
                ...
                MAX(CASE WHEN l.lap_number = ? THEN l.timestamp END) as finish_timestamp
            FROM race_drivers rd
            JOIN drivers d ON rd.driver_id = d.id
        ...
        ''', (laps_limit, laps_limit, laps_limit, laps_limit, laps_limit, laps_limit, s_id)).fetchall()
        ...
        for row in results:
            ...
            # Manejo seguro de total_time
            total_time_val = row_dict.get('total_time')
            row_dict['total_time'] = float(total_time_val) if total_time_val is not None and not (isinstance(total_time_val, str) and not total_time_val.strip()) else 0.0
            
            # Manejo seguro de finish_total_seconds
            finish_total_val = row_dict.get('finish_total_seconds')
            row_dict['finish_total_seconds'] = float(finish_total_val) if finish_total_val is not None and not (isinstance(finish_total_val, str) and not finish_total_val.strip()) else 0.0
            ...
            if last_lap_row:
                last_lap_dict = dict(last_lap_row)
                lap_seconds_val = last_lap_dict.get('lap_seconds')
                row_dict['last_lap'] = float(lap_seconds_val) if lap_seconds_val is not None and not (isinstance(lap_seconds_val, str) and not lap_seconds_val.strip()) else None
                
                avg_speed_val = last_lap_dict.get('avg_speed_kmh')
                row_dict['avg_speed_kmh'] = float(avg_speed_val) if avg_speed_val is not None and not (isinstance(avg_speed_val, str) and not avg_speed_val.strip()) else None
            else:
                row_dict['last_lap'] = None
                row_dict['avg_speed_kmh'] = None
            
            # Guardar el finish_timestamp para usar en el ordenamiento
            row_dict['finish_timestamp'] = row_dict.get('finish_timestamp')
                
            leaderboard.append(row_dict)
        
        # ========== ORDENAMIENTO SEGÚN MODO DE CARRERA ==========
        if race_mode == 'time_attack':
            # TIME ATTACK: Gana el que tiene MENOR tiempo total (completando todas las vueltas)
            # Primero los que terminaron (por tiempo total), luego los que no terminaron (por vueltas)
            leaderboard.sort(key=lambda x: (
                0 if x['is_finished'] else 1,  # Primero los que terminaron
                x['total_time'] if x['is_finished'] and x['total_time'] > 0 else float('inf'),  # Menor tiempo primero
                -x['total_laps'],  # Si no terminaron, más vueltas primero
                x['best_lap'] if x['best_lap'] else 999999  # Desempate por mejor vuelta
            ))
        else:
            # POSITION RACE: Gana el que completa las vueltas PRIMERO (cronológicamente)
            # Ordenar por: primero los que terminaron → quién terminó antes (finish_timestamp) → más vueltas
            leaderboard.sort(key=lambda x: (
                0 if x['is_finished'] else 1,  # Primero los que terminaron
                x['finish_timestamp'] if x['is_finished'] and x['finish_timestamp'] else '9999-12-31T23:59:59',  # Quién terminó ANTES cronológicamente
                -x['total_laps'],  # Si no terminaron, más vueltas primero
                x['best_lap'] if x['best_lap'] else 999999  # Desempate por mejor vuelta
            ))
...
def get_podium(session_id):
    with get_db() as conn:
        ...
        session = conn.execute('SELECT race_mode, laps_limit FROM race_sessions WHERE id = ?', (s_id,)).fetchone()
        ...
        # Obtener todos los datos necesarios para el podium
        rows = conn.execute('''
            SELECT
                d.id AS driver_id,
                d.name,
            ...
            WHERE rd.session_id = ?
            GROUP BY rd.id, d.id, d.name, d.lastname, d.transponder_id, t.kart_id, d.photo
        ''', (laps_limit, laps_limit, laps_limit, laps_limit, laps_limit, s_id)).fetchall()
        ...
        # ========== ORDENAMIENTO PARA PODIO SEGÚN MODO ==========
        if race_mode == 'time_attack':
            # TIME ATTACK: Ordenar por tiempo total (menor primero), solo los que terminaron
            ...
            # POSITION RACE: Ordenar por quién terminó primero (finish_timestamp, cronológicamente)
            podium.sort(key=lambda x: (
                0 if x['is_finished'] else 1,  # Primero los que terminaron
                x['last_lap_ts'] if x['is_finished'] and x['last_lap_ts'] else '9999-12-31T23:59:59',  # Quién terminó ANTES cronológicamente
                -x['total_laps'],  # Si no terminaron, más vueltas primero
                x['best_lap'] if x['best_lap'] else 999999
            ))
...
def guardar_estado_repetir(session_id, circuit_name, laps_limit, race_drivers, race_mode=None):
    estado = {
        "action": "repeat_race",
    ...
    with get_db() as conn:
        ...
        for row in results:
            ...
            times.append({
                ...
                'individual_time_formatted': format_individual_time(individual_time) if individual_time else '--',  # Ya existe
...
def soft_reset_race_data():
    ...
    with get_db() as conn:
        ...
        sessions_count = conn.execute('SELECT COUNT(*) FROM race_sessions').fetchone()[0]
        race_drivers_count = conn.execute('SELECT COUNT(*) FROM race_drivers').fetchone()[0]
        
        print(f"📊 Borrando: {laps_count} vueltas, {sessions_count} carreras, {race_drivers_count} inscripciones")
...
def backup_drivers_and_transponders():
    ...
    with get_db() as conn:
        ...
        transponders = [dict(row) for row in conn.execute('SELECT * FROM transponders').fetchall()]
    ...
    backup_dir = get_backup_dir()
    filename = f"pilotos_transponders_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    with get_db() as conn:
        ...
        for d in drivers:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (d['id'], d['transponder_id'], d['name'], d.get('lastname', ''),
                  d.get('age'), d.get('gender', ''), d.get('nationality', ''),
                  d.get('weight'), d.get('description', ''), d.get('photo', 'default-avatar.png'),
                  d.get('best_lap_time'), d.get('total_races', 0), d.get('total_wins', 0),
                  d.get('created_at')))
        ...
        for t in transponders:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (t['id'], t.get('code', str(t['id'])), t.get('description', ''),
                  t.get('kart_id', ''), t.get('is_active', 1), t.get('first_detected'),
                  t.get('last_seen'), t.get('times_detected', 1),
                  t.get('last_signal_h'), t.get('last_signal_l'),
                  t.get('last_time_accumulated'), t.get('last_physical_laps'),
    ...
    try:
        with get_db() as conn:
            ...
            conn.execute('''
                ...
                VALUES (1, 'server', 5.0)
            ''')
            print("[TIMING] Configuración inicializada: time_source='server', min_lap=5.0s")
    ...
    try:
        with get_db() as conn:
            if time_source is not None:
                conn.execute('UPDATE timing_config SET time_source = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1', (time_source,))
            if min_valid_lap_time is not None:
                conn.execute('UPDATE timing_config SET min_valid_lap_time = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1', (min_valid_lap_time,))
            return True
    except Exception as e:
...
def get_track_length():
    ...
    with get_db() as conn:
        result = conn.execute('SELECT track_length_km FROM circuit_config WHERE id = 1').fetchone()
...
def get_circuit_config():
    ...
    with get_db() as conn:
        result = conn.execute('SELECT track_length_km, track_type FROM circuit_config WHERE id = 1').fetchone()
        if result:
            return {
                'track_length_km': float(result['track_length_km']) if result['track_length_km'] else 0.0,
