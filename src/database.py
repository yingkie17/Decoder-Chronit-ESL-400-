...
def get_leaderboard_with_details(session_id):
    with get_db() as conn:
        ...
        laps_limit = (
            session_info["laps_limit"]
            if session_info and session_info["laps_limit"]
            else 10
        )

        race_mode = (
            normalize_race_mode(session_info["race_mode"])
            if session_info
            else "position"
        )
        laps_limit = (
...
""",
            (laps_limit, laps_limit, laps_limit, s_id),
        ).fetchall()
        ...
        if race_mode == "time_attack":
            ...
            finished.sort(
                key=lambda x: (
                    x["real_total_time"] if x["real_total_time"] > 0 else float("inf")
                )
            )
            ...
            # Ordenar terminados por finish_timestamp (quien terminó ANTES primero)
            finished.sort(
                key=lambda x: (
                    x["finish_timestamp"]
            ...
            # Ordenar no terminados por vueltas DESC, luego mejor vuelta ASC
            not_finished.sort(
                key=lambda x: (
                    -x["total_laps"],
                    x["best_lap"] if x["best_lap"] else 999999,
            ...
            # Unir: primeros los terminados, luego los no terminados
            leaderboard = finished + not_finished
        ...
        # ===== CÁLCULO DE GAP (DIFERENCIA CON EL LÍDER) =====
        if leaderboard and len(leaderboard) > 0:
            ...
            for driver in leaderboard:
                if driver["driver_id"] == leader["driver_id"]:
                    ...
                    # Misma cantidad de vueltas, diferencia de tiempo
                    if race_mode == "time_attack":
                        leader_time = leader.get("real_total_time", 0) or 0
                        driver_time = driver.get("real_total_time", 0) or 0
                    else:
                    ...
                    if leader_time > 0 and driver_time > 0:
                        ...
                        driver["gap"] = (
                            f"+{gap_seconds:.3f}s" if gap_seconds > 0 else "Líder"
                        )
                    else:
                        driver["gap"] = "--"
