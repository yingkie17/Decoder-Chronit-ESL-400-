...
def get_leaderboard_with_details(session_id):
    with get_db() as conn:
        ...
        if race_mode == 'time_attack':
            ...
            # POSITION RACE: Gana el que completa las vueltas PRIMERO
            # Ordenar por: primero los que terminaron → quién terminó antes (finish_total_seconds) → más vueltas
            leaderboard.sort(key=lambda x: (
                0 if x['is_finished'] else 1,  # Primero los que terminaron
                x['finish_total_seconds'] if x['is_finished'] and x['finish_total_seconds'] > 0 else float('inf'),  # Quién terminó antes
                -x['total_laps'],  # Si no terminaron, más vueltas primero
                x['best_lap'] if x['best_lap'] else 999999  # Desempate por mejor vuelta
            ))
...
def get_podium(session_id):
    with get_db() as conn:
        ...
        if race_mode == 'time_attack':
            ...
            # POSITION RACE: Ordenar por quién terminó primero (finish_total_seconds) o más vueltas
            podium.sort(key=lambda x: (
                0 if x['is_finished'] else 1,  # Primero los que terminaron
                x['finish_total_seconds'] if x['is_finished'] and x['finish_total_seconds'] > 0 else float('inf'),
                -x['total_laps'],  # Si no terminaron, más vueltas primero
                x['best_lap'] if x['best_lap'] else 999999
            ))
